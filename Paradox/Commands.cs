...
namespace Paradox.Commands
{
    public class Commands : BaseCommandModule
    {
        ...
        static string Trim(ref string input, int lengthToTrim)
        {
            if (string.IsNullOrEmpty(input) || lengthToTrim <= 0)
                return string.Empty;

            if (lengthToTrim >= input.Length)
            {
            ...
            string removedPart = input.Substring(input.Length - lengthToTrim);
            input = input.Substring(0, input.Length - lengthToTrim);
            return removedPart;
        }
        ...
        async void ChunkGenerate(CommandContext ctx, string prompt)
        {
            var response = "";
        ...
        async void ChunkRespond(CommandContext ctx, string response)
        {
            ...
            history.Add(new Content(response, Roles.Model));
